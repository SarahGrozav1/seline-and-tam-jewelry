...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect(reverse('products:detail_product',
                            args=[product.id]))
        else:
...
def submit_review(request, product_id):
    if request.method == 'POST':
        ...
        ReviewRating.objects.create(user=user_profile, product=product,
                                    comment=comment, rating=rating)
        messages.success(request, 'Your review has been submitted!')
    else:
        messages.error(request, 'Failed to add a review. \
        The stars are required.')
        ...
        if request.user.id == review.user.id:
            if request.method == "POST":
                ...
                if form.is_valid():
                    ...
                    return redirect("products:detail_product",
                                    product_id=product_id)

                return render(request, 'products/editreview.html',
                              {"form": form})
            else:
...
def add_to_wishlist(request, product_id):
    ...
    # Retrieve the user's wishlist from the database or create a new
